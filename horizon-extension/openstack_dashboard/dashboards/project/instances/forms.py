...
from openstack_dashboard.dashboards.project.instances import utils as project_utils
...
class ConfigurationForm(forms.SelfHandlingForm):
    '''
    Configuration form, used to configure the VNFs
    '''
    ...
    def __init__(self, request, *args, **kwargs):
        super(ConfigurationForm, self).__init__(request, *args, **kwargs)
        if 'extra' in kwargs['initial']:
            extra = kwargs['initial']['extra']

        else:
            extra = []
        self.kwargs = kwargs
        self._populate_initial(extra)

    def _populate_initial(self, extra):
        for field in extra:
            if field['type'] == 'string':
                self.fields[field['id']] = forms.CharField(label=field['label'], initial=field['default'],required=False)
            if field['type'] == 'enumeration':
                self.fields[field['id']] = forms.CharField(label=field['label'], widget=forms.Select(choices=field['enumeration']), initial=field['default'])
    ...
    def handle(self, request, data):
        try:
            instance_id = self.kwargs['initial']['instance_id']
            json_instance, exit_code, output = project_utils.FormFields(instance_id).create_configuration_instance(data)
            LOG.info("VALIDATION - error code: "+str(exit_code)+" ourput: "+str(output))
            if exit_code is not None:
                raise forms.ValidationError(output)
            project_utils.send_configuration(instance_id, json_instance)
            messages.success(request,"Instance configured")
            return True
        except forms.ValidationError:
            redirect = reverse('horizon:project:instances:index')
            exceptions.handle(request,_('Validation error.'), redirect=redirect)
            return False
