...
class ConfigurationTab(tabs.Tab):
    '''
    Configuration tab, used to configure the VNFs
    '''
    __author__ = "Fabio Mignini"
    name = _("Configuration")
    slug = "configuration"
    template_name = "project/instances/_detail_configuration.html"
    preload = False
    
    def get_context_data(self, request):
        instance = self.tab_group.kwargs['instance']
        data = _('Unable to get log for instance "%s".') % instance.id
        form = project_forms.ConfigurationForm(request=request, extra=['ipaddress', 'default_gw'])
        return {'form': form, 'instance_id': instance.id}
"""

class InstanceDetailTabs(tabs.TabGroup):
    slug = "instance_details"
    #tabs = (OverviewTab, ConfigurationTab, LogTab, ConsoleTab)
